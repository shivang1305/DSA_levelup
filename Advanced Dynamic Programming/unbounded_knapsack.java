...
public class unbounded_knapsack {
    public static void main(String[] args) {
        ...
        System.out.println(memoizationSol(W, wt, val, n, dp));
    }

    public static int recursiveSol(int W, int[] wt, int[] val, int n) {
        ...
        if(wt[n - 1] <= W) // we can only put an item in the bag if the left over capacity of the bag is less more than equal to weight of the item in the array
            return Math.max(val[n - 1] + recursiveSol(W - wt[n - 1], wt, val, n), recursiveSol(W, wt, val, n - 1)); // return the max of both cases (if we put the element in the bag or not)
        
        else
            return recursiveSol(W, wt, val, n - 1); // no ki call
    }
    ...
    public static int memoizationSol(int W, int[] wt, int[] val, int n, int[][] dp) {
        if(n == 0 || W == 0)
            return 0;
        ...
        if(wt[n - 1] <= W) 
            return dp[n][W] = Math.max(val[n - 1] + memoizationSol(W - wt[n - 1], wt, val, n, dp), memoizationSol(W, wt, val, n - 1, dp));
        
        else
            return dp[n][W] = memoizationSol(W, wt, val, n - 1, dp);
    }

    public static int tabulationSol(int W, int[] wt, int[] val, int n, int[][] dp) {
